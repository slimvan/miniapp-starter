...
class WxCountUp {
  constructor(target = '', endVal = 0, options = {}, context = {}) {
    ...
    this.count = (timestamp) => {
      ...
      if (this.useEasing) {
          if (this.countDown) {
              this.frameVal = this.easingFn(progress, 0, this.startVal - this.endVal, this.duration);
          }
          else {
    ...
    this.formatNumber = (num) => {
        ...
        if (this.options.numerals && this.options.numerals.length) {
            x1 = x1.replace(/[0-9]/g, (w) => { return this.options.numerals[+w]; });
            x2 = x2.replace(/[0-9]/g, (w) => { return this.options.numerals[+w]; });
        }
